...
```
[Start] --> [LLM: Classify] --> [IF/ELSE] --> [LLM: Respond] --> [Answer]
  |              |                  |                |                |
  query     classification      "support"?       response          output
              result            |-- Yes --> [Knowledge Retrieval] --> [LLM: Support]
                                |                                       |
                                '-- No  --> [LLM: General] ----------> [Answer]
```
...
```
                                 +-- IF (support) ---+
                                 |                    |
[Start] --> [LLM: Classify] --> [IF/ELSE]           [Knowledge] --> [LLM: Support] --> [Answer]
                                 |                    
                                 +-- ELSE (general) -+
                                                      |
                                            [LLM: General] --> [Answer]
