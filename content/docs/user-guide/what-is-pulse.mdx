...
```
[Receive Question] --> [Search Knowledge Base] --> [Ask AI] --> [Show Answer]
```
...
+-- Workspace --------------------------------------------------+
|                                                                |
|  +-- Model Providers ---+   +-- Knowledge Bases ---+          |
|  |  OpenAI              |   |  Company Docs        |          |
|  |  Anthropic           |   |  Product Manual      |          |
|  |  Google AI           |   |  FAQ Database        |          |
|  +-----------+----------+   +-----------+----------+          |
|              |                          |                      |
|              v                          v                      |
...
|  |                                                       |     |
|  |  Workflow:                                            |     |
|  |  [Start] -> [Search KB] -> [LLM] -> [Answer]         |     |
|  |                                                       |     |
|  |  Uses: GPT-4 model + Company Docs knowledge base     |     |
|  +-------------------------------------------------------+     |
|                                                                |
|  +-- App: Weekly Report Generator ----------------------+     |
|  |                                                       |     |
|  |  Mode: Workflow (scheduled, runs every Monday)        |     |
|  |                                                       |     |
|  |  Workflow:                                            |     |
|  |  [Schedule Trigger] -> [HTTP: Get Data] -> [LLM:     |     |
|  |   Summarize] -> [HTTP: Send Email]                    |     |
|  +-------------------------------------------------------+     |
|                                                                |
+----------------------------------------------------------------+
```
