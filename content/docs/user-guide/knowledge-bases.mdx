...
```
1. You upload a document (e.g., "Product Manual.pdf")
                |
                v
2. Pulse reads the document and breaks it into smaller pieces
   (like cutting a book into individual paragraphs)
                |
                v
3. Pulse converts each piece into a format it can search quickly
   (like creating an index at the back of a book)
                |
                v
4. When a user asks a question, Pulse searches the index
   to find the most relevant pieces
                |
                v
5. Those relevant pieces are sent to the AI along with the question
                |
                v
6. The AI uses the information to give a specific, accurate answer
```
...
Original document (10 pages)
     |
     v
Chunk 1: "Introduction to our product line..."  (first 500 words)
Chunk 2: "Pricing and availability..."          (next 500 words)
Chunk 3: "Technical specifications..."          (next 500 words)
... and so on
```
