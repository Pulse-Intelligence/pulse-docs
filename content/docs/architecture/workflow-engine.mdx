...
|                                                                  |
|  +------------------+    +------------------+    +-------------+ |
|  |   WorkerPool     |    |   ReadyQueue     |    | EventQueue  | |
|  |  (thread pool)   |--->|  (thread-safe)   |    | (stdlib)    | |
|  +--------+---------+    +------------------+    +------+------+ |
|           |                                             |        |
|           | execute node                    push events |        |
|           v                                             v        |
|  +------------------+    +------------------+    +-------------+ |
|  |  Worker threads  |    |   Dispatcher     |--->| EventHandler| |
|  |  pull from queue  |    |  (event loop)   |    | (routing)   | |
|  +------------------+    +--------+---------+    +------+------+ |
|                                   |                     |        |
|                          +--------v---------+    +------v------+ |
|                          | ExecutionCoord.  |    |EdgeProcessor| |
|                          | (lifecycle mgmt) |    |(graph trav.)| |
|                          +------------------+    +------+------+ |
|                                                         |        |
|                          +------------------+    +------v------+ |
|                          | CommandProcessor |    |SkipPropagat.| |
|                          | (external cmds)  |    |(skip logic) | |
|                          +------------------+    +-------------+ |
|                                                                  |
|  +------------------+    +------------------+    +-------------+ |
|  | ResponseStream   |    | GraphState       |    | ErrorHandler| |
|  | Coordinator      |    | Manager          |    | (strategies)| |
|  +------------------+    +------------------+    +-------------+ |
|                                                                  |
|  +------------------+    +------------------+                    |
|  | EventManager     |    | Layers (plugins) |                    |
|  | (collect+emit)   |    | [Debug,Limits,.] |                    |
|  +------------------+    +------------------+                    |
+------------------------------------------------------------------+
```
...
GraphEngine.run()
    ...
    |       |
    |       +-> Register response nodes with coordinator
    |       |
    |       +-> StateManager.enqueue_node(root_node.id)
    |       |       Put root node into ReadyQueue
    |       |
    |       +-> Dispatcher.start()
    |               Start event dispatch loop thread
    |
    +-> yield from EventManager.emit_events()
    |       Yield events to caller as they arrive
    |
    +-> Handle completion (success/failure/partial/paused/aborted)
    |
    +-> _stop_execution()
...
```
Worker Thread           EventQueue          Dispatcher          EventHandler
    |                      |                    |                    |
    |-- node.run() ------->|                    |                    |
    |   yields events      |                    |                    |
    |-- put(event) ------->|                    |                    |
...
```
                    ...
                    +--+--------+--------+--+
                       |        |        |
                  NodeRunResult | Generator
                       |        |    of events
                       v        v        |
              +--------+--+  +-+--------+--------+
              | Convert   |  | For each event:   |
              | to event  |  |   dispatch(event) |
              +-----------+  +---+--------+------+
                                 |        |
                    StreamChunk  |  StreamCompleted
                    AgentLog     |  PauseRequested
                    IterEvents   |  LoopEvents
                                 v
                         NodeRunSucceeded
                              or
                         NodeRunFailed
...
+-------------------------------------+----------------------------------+
| "sys"                               | System variables (query, files,  |
...
+-------------------------------------+----------------------------------+
| "<actual_node_id>"                  | Node outputs (set after          |
...
Node Failure
    |
    v
+-- Has retry config AND retries remaining?
|       YES -> sleep(retry_interval) -> NodeRunRetryEvent -> re-execute
|       NO  -> continue to error strategy
|
+-- Node error_strategy setting?
        |
        +-- None (default) -> ABORT
        |       Stop entire graph execution
        |
        +-- FAIL_BRANCH -> NodeRunExceptionEvent
        |       Route through "fail-branch" edge
        |       Outputs: {error_message, error_type}
        |
        +-- DEFAULT_VALUE -> NodeRunExceptionEvent
                Use node's default_value_dict as outputs
...
```
              ...
              true   /     \   false
                   /         \
              +---v---+   +---v---+
              | LLM_A |   | LLM_B |  <-- SKIPPED if condition is true
              +---+---+   +---+---+
                  |             |
                  v             v
              +---+-------------+---+
              |     ANSWER          |  <-- Only runs when all
