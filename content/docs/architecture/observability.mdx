...
```
+--------------------+     +------------------+     +------------------+
|  OpenTelemetry     |     |  Sentry          |     |  LLM Tracers     |
|  (traces, metrics) |     |  (errors)        |     | (Langfuse, etc.) |
+--------+-----------+     +--------+---------+     +--------+---------+
         |                          |                        |
         v                          v                        v
+--------+------------+    +--------+---------+    +---------+----------+
| OTLP Collector      |    | Sentry Cloud     |    | Langfuse / Opik /  |
| (Jaeger, Grafana,   |    | or Self-hosted   |    | Langsmith          |
|  Honeycomb, etc.)   |    +------------------+    +--------------------+
+---------------------+
```
...
GraphEngine
  ...
  +-- WorkerPool
       ...
       +-- Worker executes node
            |
            +-- ObservabilityLayer.before_node_execution()
            ...
            |       - Redis operations
            |
            +-- ObservabilityLayer.after_node_execution()
...
[Workflow Run Span]
  ...
  +-- [LLM Node Span]
  |     |
  |     +-- [httpx: POST openai.com/v1/chat/completions]
  |
  +-- [IF/ELSE Node Span]
  |
  +-- [Knowledge Retrieval Node Span]
  |     |
  |     +-- [SQLAlchemy: SELECT from document_segments]
  |     +-- [httpx: POST embeddings API]
  |     +-- [Redis: GET cached_embedding]
  |
  +-- [Answer Node Span]
```
...
```
Flask API (request span)
  |
  +-- Enqueue Celery task (with trace context in headers)
       |
       v
Celery Worker (task span, linked to request span)
  |
  +-- GraphEngine (workflow span)
       |
       +-- Node spans (with inherited context)
```
